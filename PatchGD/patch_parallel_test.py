...
def print_gradients(module, grad_input, grad_output):
    print("Grad Input",[torch.sum(grad_input[i]) for i in range(len(grad_input))])
    print("Grad Output",torch.sum(grad_output[0]))
    
def reduce_gradients(module, grad_input, grad_output):
    all_reduce(grad_output[0])
    print("Reduced Gradients",torch.sum(grad_output[0]))

class UnusedParam(nn.Module):
    def __init__(self, model, hidden_size, output_size):
        super(UnusedParam, self).__init__()
        self.model = model
        self.unused_layer = nn.Linear(hidden_size, output_size).to("cuda")
        self.tensor = torch.Tensor(1,hidden_size).to('cuda')
    
    def forward(self, x):
        out = self.model(x)
        out2 =self.unused_layer(self.tensor)
        return out
