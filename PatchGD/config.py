...
PATCH_SIZE = 128
SCALE_FACTOR = 1 
IMAGE_SIZE = 256*SCALE_FACTOR
RESIZE= True
WARMUP_EPOCHS = 2
if DISTRIBUTE_TRAINING_PATCHES == False:
    PATCH_BATCHES = math.ceil(1/PERCENT_SAMPLING)
else:
    PATCH_BATCHES = math.ceil(1/PERCENT_SAMPLING)/NUM_GPUS
...
EXPT_NAME = BACKBONE_MODEL+"_ImgSize"+str(IMAGE_SIZE)+"_BS"+str(BATCH_SIZE)+"_IBS"+str(int(((int(IMAGE_SIZE/PATCH_SIZE))**2)*PERCENT_SAMPLING))+"_PS"+str(PATCH_SIZE)+"_"+str(EPSILON)+"IB-Grad_WD-"+str(OPTIMIZER_WEIGHT_DECAY)+"-HeadType-"+str(HEAD_TYPE)+"-PatchParallel"
