...
class Trainer():
    def __init__(self,
        ...
        self.model1 =  DeiT_Backbone(self.latent_dimension)
        #checkpoint = torch.load("/home/raghavmagazine/PatchGD-main/pandas_baseline/ckpt/best_val_accuracy.pt")
        #self.model1.load_state_dict(checkpoint['model1_weights'])
        self.model1.to(self.accelarator)
        print("Usualy")
        para_1 = sum(param.numel() for param in self.model1.parameters())
        print("Number of Parameters for Backbone : ",para_1)
        for param in self.model1.parameters():
            param.requires_grad = True

        #self.model2 = get_head_from_name(self.head,self.latent_dimension)
        #self.model2 = ViT_small(image_size=IMAGE_SIZE//PATCH_SIZE,patch_size=2,num_classes=NUM_CLASSES,dim=LATENT_DIMENSION,depth=3,heads=4,mlp_dim=LATENT_DIMENSION//4,dropout=0.1,emb_dropout = 0.1,channels=LATENT_DIMENSION)
        self.model2 = Attentin_Model(num_patches=int(IMAGE_SIZE//PATCH_SIZE)**2,num_classes=NUM_CLASSES,dim=LATENT_DIMENSION,depth=DEPTH,heads=HEADS,mlp_dim=LATENT_DIMENSION//4,dropout=0.1,emb_dropout = 0.1)
        para_2 = sum(param.numel() for param in self.model2.parameters())
        print("Number of Parameters for Theta 2 :",para_2)
        ...
        parameters = [{'params': self.model1.parameters(),
                    ...
                    'lr': self.lrs['head']}]
        self.optimizer = optim.Adam(parameters,weight_decay=1e-2)
    ...
    def train_step(self):
        ...
        for images,labels,indices in tqdm(self.train_loader):
            ...
            num_patches = int(math.ceil(len(patch_dataset)*self.percent_sampling))
            ...
            with torch.no_grad():
                for patches, idxs in patch_loader:
                    ...
                    out = self.model1(patches,num_patches)
    ...
    def val_step(self):
        ...
        with torch.no_grad():
            ...
            for images,labels,indices in self.validation_loader:
