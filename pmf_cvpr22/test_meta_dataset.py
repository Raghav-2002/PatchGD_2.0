...
from models.model import MSA_HEAD
from config import *
...
def main(args):
    ...
    #PatchGD Head Model
    model_head = MSA_HEAD(accelarator=device,num_patches=TOTAL_NUM_PATCHES,dim=LATENT_DIM, depth=3, heads=4, mlp_dim=LATENT_DIM*4,dropout=0.1,emb_dropout = 0.1)
    ...
    if args.distributed:
        ...
        #PacthGD
        model_head = torch.nn.parallel.DistributedDataParallel(model_head, device_ids=[args.gpu],
                                                          find_unused_parameters=args.unused_params)
    ...
    for domain in datasets:
        ...
        # validate lr
        best_lr = args.ada_lr
        if args.deploy == 'finetune' or args.deploy == 'finetune_pgd':
            print("Start selecting the best lr...")
            best_acc = 0
            for lr in [0, 0.0001, 0.001, 0.01]:
                model_without_ddp.lr = lr
                test_stats = evaluate(data_loader_val, model, model_head, criterion, device, seed=1234, ep=5)
                acc = test_stats['acc1']
        ...
        test_stats = evaluate(data_loader_val, model, model_head,criterion, device)
