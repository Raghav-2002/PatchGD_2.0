...
def evaluate(data_loaders, model,model_head, criterion, device, seed=None, ep=None):
    if isinstance(data_loaders, dict):
        ...
        for j, (source, data_loader) in enumerate(data_loaders.items()):
            ...
            test_stats = _evaluate(data_loader, model,model_head, criterion, device, seed_j)
        ...
        return _evaluate(data_loaders, model,model_head, criterion, device, seed, ep)
    else:
        warnings.warn(f'The structure of {data_loaders} is not recognizable.')
        return _evaluate(data_loaders, model,model_head, criterion, device, seed)


@torch.no_grad()
def _evaluate(data_loader, model, model_head, criterion, device, seed=None, ep=None):
    metric_logger = utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('n_ways', utils.SmoothedValue(window_size=1, fmt='{value:d}'))
    metric_logger.add_meter('n_imgs', utils.SmoothedValue(window_size=1, fmt='{value:d}'))
    ...
    for ii, batch in enumerate(metric_logger.log_every(data_loader, 1, header)):
        ...
        copy_mode_head = copy.deepcopy(model_head)
        copy_mode_head.load_state_dict(model_head.state_dict())

        # compute output
        with torch.cuda.amp.autocast():
            output = model(SupportTensor, SupportLabel, x, copy_mode_head)
